...
NS = Env.new({
  ...
  :nth => func { |list, n| n < list.count ? list[n] : fail('not found') },
  :first => func { |list| list[0] },
  :rest => func { |list| list ? MalList.new(list.drop(1)) : MalList.new },
  ...
  :apply => func { |f, *args| args ||= []; list = args.pop || []; f.call(*(args + list)) },
  :map => func { |f, list| MalList.new(list.map { |x| f.call(x) }) },
  :readline => func { |prompt| Readline.readline(prompt, true) },
  :atom => func { |x| MalAtom.new(x) },
  :atom? => func { |x| x.is_a? MalAtom },
  :deref => func { |x| x.val },
  ...
  :'time-ms' => func { (Time.now.to_f * 1000).to_i },
})
